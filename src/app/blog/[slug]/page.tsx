...
export default function BlogDetailPage() {
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!session || !commentText.trim() || submittingComment || !blog) return;
  ...
  if (!blog) {
    return (
      <div className="min-h-screen bg-[#030014] text-slate-400 flex flex-col items-center justify-center p-6">
        <p className="text-xs mb-3">Blog article not found.</p>
        <button onClick={() => router.push("/blog")} className="text-brand-cyan text-xs underline">
          Return to Blog Feed
        </button>
      </div>
    );
    ...
    <div className="flex flex-col min-h-screen bg-[#030014]">
      ...
      <main className="flex-grow mx-auto max-w-5xl w-full px-4 py-8 sm:px-6 relative z-10">
        ...
        <button
          ...
          className="text-xs text-slate-500 hover:text-white flex items-center space-x-1 mb-6"
        >
