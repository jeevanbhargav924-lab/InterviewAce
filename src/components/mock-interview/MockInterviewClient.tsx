...
import { Cpu, Play, CheckSquare, AlertCircle } from "lucide-react";
...
export default function MockInterviewClient() {
  ...
  return (
    <div className="w-full max-w-xl bg-glass border border-slate-800 rounded-2xl p-8 shadow-2xl relative text-left">
      ...
      <form onSubmit={handleStartInterview} className="space-y-6">
              ...
              <button
                ...
                className={`text-left rounded-lg p-4 border transition-all cursor-pointer ${
                  topic === role.name
                    ...
                    : "border-slate-800 bg-slate-955/40 hover:border-slate-700"
                }`}
              >
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full rounded-lg bg-gradient-to-r from-brand-purple to-brand-cyan py-3.5 text-xs font-semibold text-white shadow-xl hover:brightness-110 active:scale-95 transition-all flex items-center justify-center space-x-1 disabled:opacity-50 cursor-pointer"
        >
