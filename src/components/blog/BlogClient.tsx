...
export default function BlogClient({ initialBlogs }: BlogClientProps) {
  ...
  useEffect(() => {
    // Skip initial fetch on mount since we have initialBlogs
    ...
    loadBlogs();
  }, [selectedCategory, searchQuery, initialBlogs]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center space-x-2">
            <span>{selectedCategory === "all" ? "All Publications" : `${selectedCategory} Articles`}</span>
          </h2>
          {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-glass rounded-xl p-5 border border-slate-800 text-center">
            <CrownIcon />
            <h2 className="text-xs font-bold text-white mt-2">Become a Sponsored Creator</h2>
